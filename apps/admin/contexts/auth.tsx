...
import { CurrentUserQuery, useCurrentUserLazyQuery, useSignInUserMutation } from 'types/graphql'
...
const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const [siginInUserMutation] = useSignInUserMutation()
  ...
  const signIn = async (email: string, password: string) => {
    setLoading(true)
    const { data, errors } = await siginInUserMutation({
      variables: {
        data: {
          email,
          password,
