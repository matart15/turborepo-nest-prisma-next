...
moduleとしては、再利用性の高い単位で切り出しておいて、複数のmoduleから呼び出される前提で実装する。
...
自動生成しているzodを使って作っているEntity(yarn generate:repository コマンドでインタラクティブにentityなどを作成)
...
これはprismaをそのまま使用しているので何でもできてしまうインスタンス。
